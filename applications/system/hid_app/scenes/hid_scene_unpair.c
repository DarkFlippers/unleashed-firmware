...
void hid_scene_unpair_on_enter(void* context) {
    ...
    dialog_ex_set_header(app->dialog, "¿Olvidar todos los disp.?", 64, 3, AlignCenter, AlignTop);
    dialog_ex_set_text(
        app->dialog, "Todos los disp.\nSe desemparejaran", 64, 22, AlignCenter, AlignTop);
    dialog_ex_set_left_button_text(app->dialog, "Volver");
    dialog_ex_set_right_button_text(app->dialog, "Aceptar");
