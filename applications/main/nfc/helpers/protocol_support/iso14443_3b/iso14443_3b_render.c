...
void nfc_render_iso14443_3b_info(
    ...
    // routines into the app image; fap_exclude_libs=["gcc"] now resolves those from the firmware
    // instead, so this avoids the runtime calls rather than the bytes - and prints the exact value
    // where %4.2g gave two significant figures. fc / 13.56 MHz -> us, i.e. fc * 25 / 339. fwi is
    // capped at 14, so fc <= 4096 << 14 and the multiply fits uint32_t.
