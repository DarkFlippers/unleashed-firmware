...
void nfc_scene_slix_unlock_on_enter(void* context) {
    nfc_protocol_support_extra_on_enter(SlixExtraSceneUnlock, context);
}

bool nfc_scene_slix_unlock_on_event(void* context, SceneManagerEvent event) {
    return nfc_protocol_support_extra_on_event(SlixExtraSceneUnlock, context, event);
}

void nfc_scene_slix_unlock_on_exit(void* context) {
    nfc_protocol_support_extra_on_exit(SlixExtraSceneUnlock, context);
}
