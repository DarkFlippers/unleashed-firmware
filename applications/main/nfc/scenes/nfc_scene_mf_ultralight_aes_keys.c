...
void nfc_scene_mf_ultralight_aes_keys_widget_callback(
...
void nfc_scene_mf_ultralight_aes_keys_on_enter(void* context) {
    ...
    widget_add_button_element(
        ...
        GuiButtonTypeCenter,
        "Add",
        nfc_scene_mf_ultralight_aes_keys_widget_callback,
        instance);
...
bool nfc_scene_mf_ultralight_aes_keys_on_event(void* context, SceneManagerEvent event) {
    NfcApp* instance = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == GuiButtonTypeCenter) {
            scene_manager_next_scene(instance->scene_manager, NfcSceneMfUltralightAesKeysAdd);
            consumed = true;
        } else if(event.event == GuiButtonTypeRight) {
            scene_manager_next_scene(instance->scene_manager, NfcSceneMfUltralightAesKeysList);
            consumed = true;
        }
    }
...
void nfc_scene_mf_ultralight_aes_keys_on_exit(void* context) {
    NfcApp* instance = context;
