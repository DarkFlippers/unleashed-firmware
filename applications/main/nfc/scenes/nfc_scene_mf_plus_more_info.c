...
void nfc_scene_mf_plus_more_info_on_enter(void* context) {
    nfc_protocol_support_extra_on_enter(NfcProtocolMfPlus, MfPlusExtraSceneMoreInfo, context);
}

bool nfc_scene_mf_plus_more_info_on_event(void* context, SceneManagerEvent event) {
    return nfc_protocol_support_extra_on_event(
        NfcProtocolMfPlus, MfPlusExtraSceneMoreInfo, context, event);
}

void nfc_scene_mf_plus_more_info_on_exit(void* context) {
    nfc_protocol_support_extra_on_exit(NfcProtocolMfPlus, MfPlusExtraSceneMoreInfo, context);
}
