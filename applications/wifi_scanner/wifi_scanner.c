...

    {
        switch (app->m_context)
        {
            ...
            case ScanMode:
            {
                uint8_t offsetY = 0;
                uint8_t offsetX = 0;
                canvas_draw_frame(canvas, 0, 0, u8g2_GetDisplayWidth(&canvas->fb), u8g2_GetDisplayHeight(&canvas->fb));
                ...
                offsetX += 5;
                offsetY += fontHeight;
                canvas_draw_str(canvas, offsetX, offsetY, app->m_currentAccesspointDescription.m_isHidden ? "(Hidden SSID)" : string_get_cstr(app->m_currentAccesspointDescription.m_accessPointName));

                offsetY += fontHeight;

                canvas_draw_str(canvas, offsetX, offsetY, string_get_cstr(app->m_currentAccesspointDescription.m_bssid));
                ...
                sprintf(string, "ENCR: %s", string_get_cstr(app->m_currentAccesspointDescription.m_secType));
                canvas_draw_str(canvas, offsetX, offsetY, string);
                ...
                canvas_draw_frame(canvas, offsetX - 6, offsetY - u8g2_GetMaxCharHeight(&canvas->fb) - 3, u8g2_GetDisplayWidth(&canvas->fb), u8g2_GetDisplayHeight(&canvas->fb));
            ...
            case MonitorMode:
            {
                ...
                canvas_draw_str(canvas, offsetX, offsetY, string_get_cstr(app->m_currentAccesspointDescription.m_accessPointName));

                offsetY += fontHeight + 2;

                canvas_draw_str(canvas, offsetX, offsetY, string_get_cstr(app->m_currentAccesspointDescription.m_bssid));

                DrawSignalStrengthBar(canvas, app->m_currentAccesspointDescription.m_rssi, 5, 5, 12, 25);
                DrawSignalStrengthBar(canvas, app->m_currentAccesspointDescription.m_rssi, u8g2_GetDisplayWidth(&canvas->fb) - 5 - 12, 5, 12, 25);
            }
                break;
            case ScanAnimation:
            {
                ...
                {
                    canvas_draw_disc(canvas, offsetX + 30 + 25 * i, u8g2_GetDisplayHeight(&canvas->fb) / 2 - 7, 5 * (app->m_animtaionCounter == i ? mutliplier : 1));
                }

                u8g2_SetFont(&canvas->fb, u8g2_font_7x13B_tr);
                //canvas_set_font(canvas, FontPrimary);
                const char* message = "Scanning";
                canvas_draw_str(canvas, u8g2_GetDisplayWidth(&canvas->fb) / 2 - canvas_string_width(canvas, message) / 2, 55, message);
            }
                break;
            case MonitorAnimation:
            {
                ...
                canvas_draw_disc(canvas, offsetX + 30, u8g2_GetDisplayHeight(&canvas->fb) / 2 - 7, 5 * (app->m_animtaionCounter == 0 ? mutliplier : 1));
                canvas_draw_disc(canvas, offsetX + 55, u8g2_GetDisplayHeight(&canvas->fb) / 2 - 7, 5 * (app->m_animtaionCounter == 1 ? mutliplier : 1));
                canvas_draw_disc(canvas, offsetX + 80, u8g2_GetDisplayHeight(&canvas->fb) / 2 - 7, 5 * (app->m_animtaionCounter == 0 ? mutliplier : 1));
        ...
        if(events & WorkerEventRx) 
        {
            ...
            if(string_size(receivedString) > 0)
            {
...
#endif // ENABLE_MODULE_POWER
                {
                    ...
                    else if (string_cmp_str(chunksArray[EChunkArrayData_Context], MODULE_CONTEXT_SCAN) == 0)
                    {
                        app->m_context = ScanMode;
                    }
                    else if (string_cmp_str(chunksArray[EChunkArrayData_Context], MODULE_CONTEXT_SCAN_ANIMATION) == 0)
                    {
                        app->m_context = ScanAnimation;
                    }
                    else if (string_cmp_str(chunksArray[EChunkArrayData_Context], MODULE_CONTEXT_MONITOR_ANIMATION) == 0)
                    {
                    ...
                    {
                        string_set(app->m_currentAccesspointDescription.m_accessPointName, chunksArray[EChunkArrayData_SSID]);
                        string_set(app->m_currentAccesspointDescription.m_secType, chunksArray[EChunkArrayData_EncryptionType]);
                        app->m_currentAccesspointDescription.m_rssi = atoi(string_get_cstr(chunksArray[EChunkArrayData_RSSI]));
                        string_set(app->m_currentAccesspointDescription.m_bssid, chunksArray[EChunkArrayData_BSSID]);
                        app->m_currentAccesspointDescription.m_channel = atoi(string_get_cstr(chunksArray[EChunkArrayData_Channel]));
                        app->m_currentAccesspointDescription.m_isHidden = atoi(string_get_cstr(chunksArray[EChunkArrayData_IsHidden]));

                        app->m_currentIndexAccessPoint = atoi(string_get_cstr(chunksArray[EChunkArrayData_CurrentAPIndex]));
                        app->m_totalAccessPoints = atoi(string_get_cstr(chunksArray[EChunkArrayData_TotalAps]));
                    }
                }
