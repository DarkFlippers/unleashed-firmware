 ...
 * to honour the user's choice - LFRFIDWorker itself starts with every target enabled, so
 * honouring it is opt-in, see lfrfid_worker_set_write_targets().
 *
 ...
 *
 * Never fails: falls back to every target enabled when the settings file is missing or was
