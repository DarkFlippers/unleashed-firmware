...
## Other changes
- Desktop: Main menu styles - Left and Right now move between the columns of the C64 and Compact layouts whenever the other column has anything in it; on the stock menu the bottom half of Compact's left column did nothing at all (by @mishamyte | Fixes #1120)
- Desktop: Main menu styles - the Vertical layout no longer streams a sideways screen to qFlipper and the mobile app, a style plugin that fails to load or ships an incomplete vtable is now reported instead of silently ignored, a style file that has gone missing no longer reads as "Default" in Settings, and the style plugin ABI is documented (by @mishamyte | Fixes #1120)
