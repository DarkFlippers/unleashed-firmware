...
## Other changes
- Desktop: Main menu styles - Left and Right no longer do nothing on the bottom of the C64 and Compact layouts, the Vertical layout no longer streams a rotated screen to qFlipper and the mobile app, a style plugin that fails to load or ships an incomplete vtable is now reported instead of ignored, and the style plugin ABI is documented (by @mishamyte | Fixes #1120)
