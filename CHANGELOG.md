...
## Other changes
- GUI: Keys pressed while an application is showing a loading animation are no longer delivered to the screen that replaces it - they were queued and replayed once the event loop began, where a stray Left or Right could change a setting the user never saw (by @mishamyte | Fixes #1124)
