...
- Desktop: **Main menu styles** - Settings -> Desktop -> Menu Style picks between Default, PS4, Wii, DSi, C64, Compact, CoverFlow and Vertical, each shipped as its own plugin - layouts ported from [Momentum Firmware](https://github.com/Next-Flip/Momentum-Firmware) (by @apfxtech | PR #1119)
...
- Apps: Build tag (**3sep2026**) - **Check out more Apps updates and fixes by following** [this link](https://github.com/xMasterX/all-the-plugins/commits/dev)
## Other changes
- GUI: Applications no longer act on keys pressed while they were still starting - input that arrived before an app had a screen up was replayed into its first view once the event loop began, where a stray Left or Right could change a setting without the user ever seeing it (by @mishamyte | Fixes #1124)
- GUI: Archive and Desktop settings put a loading animation up while their first screen is built, instead of leaving the menu they were opened from on display; any app can do the same with the new view_dispatcher_show_loading(), which owns the view so it costs no view id and nothing to free (by @mishamyte | Fixes #1124)
